...
export class DeviceTemplateManagementComponent implements OnInit {
  ...
  loadStoreData() {
    if (!this.selectedStoreId) {
      this.showWarning('Please select a store first');
      return;
    }
  ...
  syncDevicesFromCloud() {
    ...
    this.deviceService.syncDevicesFromCloud(this.selectedStoreId).subscribe({
      ...
      error: (error) => {
        ...
        this.loading = false;
        this.showError('Failed to sync devices: ' + error.message);
      },
    });
  }
  ...
  filterDevices() {
    ...
    if (formValue.batteryLevel !== 'all') {
      switch (formValue.batteryLevel) {
        ...
        case 'medium':
          filtered = filtered.filter(
            (device) => device.battery > 30 && device.battery <= 70,
          );
          break;
        case 'low':
  ...
  lightUpDevice(device: any, color: number) {
    this.deviceService
      .lightUpDevice(device.mac, this.selectedStoreId, color)
      .subscribe();
  }
  ...
  syncTemplatesFromCloud() {
    ...
    this.deviceService.syncTemplatesFromCloud(this.selectedStoreId).subscribe({
      ...
      error: (error) => {
        console.error('Sync failed:', error);
        this.loading = false;
        this.showError('Failed to sync templates: ' + error.message);
      },
    });
  }
  ...
  createCombo(device: any, template: any) {
    this.deviceService
      .createDeviceTemplateCombo(device.id, template.id)
      .subscribe({
        next: (combo) => {
  ...
  createComboWithDevice(device: any) {
    ...
    dialogRef.afterClosed().subscribe((result: ComboCreationResult) => {
      if (result.success && result.combo) {
        ...
        this.showError('Failed to create combo: ' + result.error);
      }
    });
  }
  ...
  selectTemplateForCombo(device: any, combo: any) {
    ...
    dialogRef.afterClosed().subscribe((result: ComboCreationResult) => {
      if (result.success) {
        this.showSuccess('Device-Template combo created successfully!');
      }
    });
  }
  ...
  createDirectCombo(device: any, template: any) {
    if (!device || !template) {
      this.showWarning('Please select both device and template');
      return;
    }
    ...
    this.deviceService
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error('Failed to create combo:', error);
          this.showError('Failed to create combo: ' + error.message);
        },
      });
  }
    ...
    this.deviceService
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.showError('Failed to assign combo: ' + error.message);
        },
      });
  }
  ...
  loadAssignments() {
    ...
    if (formValue.locationType && formValue.locationId) {
      ...
      this.deviceService
        .getAssignments(formValue.locationType, formValue.locationId)
        .subscribe({
          next: (assignments) => {
  ...
  updateAssignmentOrder(assignmentId: number, newPosition: number) {
    ...
    this.deviceService
      .updateAssignmentOrder(assignmentId, newPosition, userId)
      .subscribe({
        next: () => {
  ...
  testBind() {
    if (this.selectedCombos.size === 0) {
      this.showWarning('Please select at least one combo');
      return;
    }

    if (!this.selectedItem || !this.selectedItemType) {
      this.showWarning('Please select an item to bind');
      return;
    }
    ...
    this.deviceService.testBindPreview(firstComboId, testData).subscribe({
      ...
      error: (error) => {
        ...
        this.showError('Test bind failed: ' + error.message);
      },
    });
  }
  ...
  applyBind() {
    ...
    const assignments = Array.from(this.selectedCombos).map((comboId) => ({
      comboId,
      productId:
        this.selectedItemType === 'Product' ? this.selectedItem.id : null,
      locationType:
        this.selectedItemType === 'Product' ? null : this.selectedItemType,
      locationId:
        this.selectedItemType === 'Product' ? null : this.selectedItem.id,
      color: 1,
      brightness: 100,
    }));
    ...
    this.deviceService.batchBind(assignments).subscribe({
      next: (response) => {
        this.showSuccess(
          `Successfully bound ${response.successCount} combo(s)`,
        );
      ...
      error: (error) => {
        ...
        this.bindingInProgress = false;
        this.showError('Bind failed: ' + error.message);
      },
    });
  }
  ...
  quickBindProduct(product: any, comboId: number) {
    this.deviceService.quickBindProduct(product.id, comboId).subscribe({
      next: () => {
        ...
        this.showSuccess('Product bound successfully');
      },
      error: (error) => {
        console.error('Bind failed:', error);
        this.showError('Bind failed: ' + error.message);
