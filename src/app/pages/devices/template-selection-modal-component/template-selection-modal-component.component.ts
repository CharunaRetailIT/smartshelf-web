...
import { TemplateSelectionModalData, ComboCreationResult } from '../../../core/interfaces/combo-create.models';
...
@Component({
  ...
  template: `
    <div class="bg-white rounded-lg shadow-xl w-full max-w-5xl mx-auto flex flex-col" style="height: 90vh;">
      <!-- Dialog Header -->
      ...
      <div class="flex flex-col flex-1 min-h-0">
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-sm text-gray-900">{{data.selectedDevice.deviceName || 'Unnamed Device'}}</span>
                    <span [class]="getStatusBadgeClass(data.selectedDevice.status)" class="px-2 py-1 rounded-full text-xs font-medium">
                  ...
                  <div class="text-xs text-gray-600 mt-2 space-y-1">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="font-mono">{{data.selectedDevice.mac}}</span>
                    </div>
                    <div class="flex items-center gap-2">
          ...
          <div class="space-y-4">
            ...
            <div *ngIf="!loading && filteredTemplates.length === 0" class="text-center py-12">
              <i class="fas fa-folder-open text-4xl text-gray-300 mb-4"></i>
              <p class="text-gray-600 font-medium">No templates found</p>
              <p class="text-sm text-gray-500 mt-1">Please sync templates from cloud first or adjust your search criteria.</p>
            </div>

            <div *ngIf="!loading && filteredTemplates.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div 
                *ngFor="let template of filteredTemplates"
                ...
                <div class="template-preview bg-gray-100 flex items-center justify-center" style="height: 160px;">
                  <img 
                ...
                <div class="p-4">
                  <div class="flex items-start justify-between mb-2">
                    <h4 class="font-medium text-sm text-gray-900 line-clamp-2 flex-1">
                  ...
                  <div class="space-y-1 text-xs text-gray-600">
                    <div class="flex items-center gap-2" *ngIf="template.screenSize">
                      <i class="fas fa-desktop text-gray-400"></i>
                      <span>{{template.screenSize?.width}}x{{template.screenSize?.height}}</span>
                    </div>
                  ...
                  <div *ngIf="selectedTemplate?.id === template.id" class="mt-3 pt-3 border-t border-blue-200">
          ...
          <div *ngIf="selectedTemplate && !loading" class="space-y-4">
            ...
            <div class="bg-green-50 border border-green-200 rounded-lg p-4">
              <div class="flex items-start gap-4">
                <div *ngIf="selectedTemplate.previewImage" class="flex-shrink-0">
                  <img 
                    [src]="selectedTemplate.previewImage" 
                    alt="Selected Template" 
                    class="w-24 h-24 object-contain rounded border border-green-300">
                </div>
                <div *ngIf="!selectedTemplate.previewImage" class="flex-shrink-0 w-24 h-24 bg-gray-100 rounded border border-green-300 flex items-center justify-center">
                  <i class="fas fa-palette text-2xl text-gray-400"></i>
                </div>
                <div class="flex-1">
                  <h4 class="font-medium text-gray-900 mb-2">{{selectedTemplate.demoName}}</h4>
                  <div class="grid grid-cols-2 gap-2 text-xs text-gray-600">
                    <div>
                      <span class="font-medium">Template ID:</span>
                      <span class="ml-1 font-mono">{{selectedTemplate.id}}</span>
                    </div>
                    <div *ngIf="selectedTemplate.screenSize">
                      <span class="font-medium">Resolution:</span>
                      <span class="ml-1">{{selectedTemplate.screenSize?.width}}x{{selectedTemplate.screenSize?.height}}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-md p-3">
              <div class="flex">
                <i class="fas fa-info-circle text-purple-400 mt-0.5 text-lg"></i>
                <div class="ml-3">
                  <p class="text-sm text-purple-700">
                    <strong>Note:</strong> Creating a combo will link this template with the selected device. The device will be able to display content using this template format.
                  </p>
                </div>
              </div>
            </div>
  ...
  loadTemplatePreviews() {
    this.templates.forEach(template => {
      this.deviceService.getTemplatePreview(template.id).subscribe({
        ...
        error: (error) => {
          console.error('Failed to load preview for template:', template.id, error);
        }
      });
    });
  }
